...
// Declare the function prototype for the user-defined function Timer 0A
void Timer_0A_Periodic_Task(void);

// Initialize a global variable for Timer 0A to keep track of elapsed tim ein miliseconds
...
// Initialize an usigned 8-bit global variable that will be
// incremented in the periodic task for Timer 0A
uint8_t count = 0;

int main(void)
{
	// Initialize the LEDs on EduBase Board (Port B)
	EduBase_LEDs_Init();
	
	// Initialize the buttons (SW2 - SW5) on the EduBase board (PortD)
	EduBase_Button_Init();
	...
	// Initialize the Seven_segment Display on the EduBase boars
...
void Timer_0A_Periodic_Task(void)
{
	// Increment the Timer_0A_ms_elapsed global variable by 1
	// for each Timer_0A generates an interrupt
	Timer_0A_ms_elapsed++;
	
	// If 500 miliseconds have passed, clear the Timer_0A_ms_elapsed to zero
